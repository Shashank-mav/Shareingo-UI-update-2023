...
app.get('*', (req, res) => {
    ...
    fs.stat(file, function (err, stat) {
        if (!err && stat.isFile()) { //is a file
            res.sendFile(file);
            return;
        }
        if (/^\/[^\/]*\//.test(req.url)) { //If the string contains two or more slashes
            req.url = req.url.match(/^\/[^\/]*/)[0]; //Truncate the string between the first two slashes
...
console.log(new Date().toISOString(), ' Shareingo is running on port', port);

const parser = require('ua-parser-js');

class ShareingoServer {
