...
Events.on('display-name', e => {
    ...
    $displayName.textContent = ' Your nickname：' + me.displayName;
...
class PeerUI {
    ...
    html() {
        return `
            <label class="column center" title="Click the icon to send the file or right-click to send the text">
...
class ReceiveTextDialog extends Dialog {
    ...
    async _onCopy() {
        ...
        Events.fire('notify-user', 'saved to clipboard');
    }
}
...
class Notifications {
    ...
    _messageNotification(message) {
        if (isURL(message)) {
            const notification = this._notify(message, 'Click to open the link');
    ...
    _downloadNotification(message) {
        const notification = this._notify(message, 'Click to save file');
        if (!window.isDownloadSupported) return;
    ...
    _copyText(message, notification) {
        ...
        this._notify(' Text copied to clipboard');
    }
...
class NetworkStatusUI {
    ...
    _showOfflineMessage() {
        Events.fire('notify-user', ' Your device is offline');
    }

    _showOnlineMessage() {
        Events.fire('notify-user', 'Your device is online');
    }
}
